...
会場名・住所・アクセス・入口／受付・駐車場・問い合わせ先は `src/lib/config.ts` に既定値を持っているため、
環境変数を設定しなくても公開画面は成立する。内容を変更する場合のみ以下を設定する。

段差・車いすでのご来館（`VENUE_ACCESSIBILITY`）は運営で未確認のため**空のまま**にしてある（推測を掲載しないため）。
未設定の項目は公開画面に表示されず、代わりにその項目名と問い合わせ先が案内される。

```
VENUE_NAME=<会場名（既定: 株式会社田中組 本社 1階ホール（指定エリア））>
VENUE_ADDRESS=<住所（既定: 北海道札幌市中央区北6条西11丁目26番地）>
VENUE_MAP_URL=<地図のURL（未設定の場合は住所からGoogleマップ検索URLを自動生成）>
VENUE_ACCESS=<最寄り駅からのアクセス（改行はそのまま表示される）>
VENUE_ENTRANCE=<建物の入口と受付場所>
VENUE_PARKING=<駐車場・駐輪場の案内>
VENUE_ACCESSIBILITY=<段差・車いすでのご来館に関する案内（未確認のため既定値は空）>
SUPPORT_PHONE=<電話での問い合わせ先（既定: 011-611-3331）>
```

確認すべき項目の詳細は [VENUE_INFO.md](./VENUE_INFO.md) を参照。
