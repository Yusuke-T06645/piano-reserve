...
会場名・住所・地図リンクは既定値を持っているため未設定でも公開画面が成立するが、
アクセス・入口・駐車場・バリアフリーの案内は運営で確認が取れるまで**空のまま**にしてある（推測を掲載しないため）。
確認が取れ次第、以下を設定すると公開画面の「会場のご案内」に表示される。未設定の項目は表示されず、代わりに問い合わせ先が案内される。
...
VENUE_ACCESS=<最寄り駅・バス停からのアクセス>
VENUE_ENTRANCE=<建物の入口と受付場所>
VENUE_PARKING=<駐車場・駐輪場の有無>
VENUE_ACCESSIBILITY=<段差・車いすでのご来館に関する案内>
SUPPORT_PHONE=<電話での問い合わせ先（設定すると各所の案内に電話番号が併記される）>
```
