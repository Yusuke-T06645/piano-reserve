...
export async function notifyReservationConfirmed(r: Reservation) {
  ...
  await getMailer().send({ to: r.email, subject, html, text, attachments: [await qrAttachment(r.checkinToken)] });
...
export async function sendManageLink(r: Reservation) {
  ...
  await getMailer().send({ to: r.email, subject, html, text });
}
...
export async function notifyWaitlistJoined(w: WaitlistEntry) {
  ...
  await getMailer().send({ to: w.email, subject, html, text });
}
...
export async function notifyAdminNewBooking(r: Reservation) {
  ...
  await getMailer().send({ to: config.adminNotifyEmail, subject, html, text });
}
